...
function OurCourses() {
  ...
  if (loading) {
    return <p className="text-center text-4xl text-blue-300">Loading...</p>;
  }
  return (
    <div className="bg-gray-100 p-8 space-y-4">
      <h1 className="text-3xl font-bold text-center mb-8">Our Courses</h1>
      <Link
        className="bg-blue-400 py-2 px-4 rounded-lg text-white hover:bg-orange-950 duration-300"
        to={"/admin/dashboard"}
      >
        Go to dashboard
      </Link>
          ...
          <div key={course._id} className="bg-white shadow-md rounded-lg p-4">
            ...
            <div className="flex justify-between">
              <Link
                ...
                className="bg-blue-400 text-white py-2 px-4 mt-4 rounded hover:bg-blue-600 duration-200"
              >
                Update
              </Link>
              <button
                onClick={() => handleDelete(course._id)}
                className="bg-white border border-red-500 py-2 px-4 mt-4 rounded hover:bg-red-600 duration-200"
              >
                Delete
              </button>
            </div>
